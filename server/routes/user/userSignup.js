...
const insertUser = async (username,email,firstName,lastName,password,db) => {
	try {
...
router.post("/user", async (req, res) => {
    ...
    if (await isEmailExist(email_lower,db))
      return res.status(400).json({error: "Email Already Exist" });
		...
		return await insertUser(username,email_lower,firstName,lastName,hashedPassword,db) ? res.status(201).json({message:"User Added Successfully"}) : res.status(500).json({error:"Error Inserting User"})
